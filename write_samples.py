...
`python write_samples.py` and read the files. This is for looking at output
with your eyes -- what the coercion pass actually emits -- not for deciding
whether it works; test.py is the judge of that.
...
120s timeout. That is test.py's job. `--check` and `--run` ask for it anyway.

  python write_samples.py                 samples_temp/, 3 masks a benchmark
  python write_samples.py --count 15      more masks each
  python write_samples.py --less-any      erase to a deleted annotation
  python write_samples.py --benchmark deltablue --benchmark nbody
  python write_samples.py --check         compile each one too
"""
...
def unit_count(bench):
    """How many annotations a mask for this benchmark chooses among.

    A benchmark whose original source will not bind has none. It still gets a
    directory holding typed_original.py, so the failure is visible rather than
    absent.
    """
    try:
...
def render(bench, mask, n, less_any):
    """The detyped source, with a header saying what produced it."""
    source = load_bench(bench, VARIANT)
    body = unparse(detype(source, mask=mask, bench=True, less_any=less_any))
    bits = bin(mask).count("1")
...
def verdict(source, run_it):
    ...
    test.py already executes. `--run` asks for it anyway; test.py is what
...
def one_sample(job):
    bench, index, mask, n, less_any, check, run_it = job
    filename = name_for(index, mask, n)
    try:
        text = render(bench, mask, n, less_any)
...
def jobs_for(bench, count, seed, less_any, check, run_it):
    n = unit_count(bench)
    masks = sample_masks(n, count, seed)
    out = [(bench, i, mask, n, less_any, check, run_it)
           for i, mask in enumerate(masks, 1)]
    if n:
        # index 0 is the max file: everything erased, always worth seeing
        out.append((bench, 0, (1 << n) - 1, n, less_any, check, run_it))
    return n, out
...
def main():
    ...
    out_dir = args.out or ("samples_temp_less_any" if args.less_any
                           else "samples_temp")
    ...
    for index, bench in enumerate(benches):
        n, bench_jobs = jobs_for(bench, args.count, args.seed + index,
                                 args.less_any, args.check or args.run,
                                 args.run)
        shapes[bench] = n
    ...
    header = (f"per-benchmark masks, {GRANULARITY} granularity, "
              f"seed={args.seed}{', less-any' if args.less_any else ''}")
