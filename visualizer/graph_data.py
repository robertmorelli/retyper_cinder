...
from src.annotation_remover import remove_annotations
from src.cinderx_binding import get_ast_data
from src.import_adder import add_imports
from src.mediate_ast import mediate_tree
from src.type_graph import TypeGraph
from src.type_rules import readable_type_name
...
def graph_data(source, mask=0, bench=False):
    ...
    graph = TypeGraph(bound, mask, granularity)
    placed = {}
    active_edges = set(graph.edges)
    if mask:
        bound = SimpleNamespace(types=graph.types,
                                type_constraints=graph.type_constraints,
                                tree=bound.tree)
        detyped = remove_annotations(written.tree, graph.erased, graph.types,
                                     graph.type_constraints)
        detyped = mediate_tree(detyped, graph.types, graph.type_constraints,
    ...
    def value(cell):
        table = bound.types if cell[1] == "type" else bound.type_constraints
    ...
    nodes = [{"id": ids[cell], "slot": cell[1],
              ...
              "label": label(cell),
              "values": [readable_type_name(value(cell))]
