...
if str(REPOSITORY) not in import_path:
    import_path.insert(0, str(REPOSITORY))

from utilities.list_benchmarks import get_benchmark_locations
from utilities.load_source import ROOT as SOURCE_ROOT
...
svg{position:fixed;inset:0;width:100%;height:100%;z-index:2;pointer-events:none}.edge{fill:none;stroke:#78889b;stroke-width:2;opacity:.28;marker-mid:url(#arrow)}.edge.hot{stroke:#ffd166;opacity:1;marker-mid:url(#arrow-hot)}body.selected .edge:not(.hot){opacity:.05}.cell{pointer-events:stroke;cursor:pointer;stroke-width:4;stroke-linecap:round}.type{stroke:#56b4ff}.type_constraint{stroke:#ff9561}
...
def benchmark_sources():
    ...
    return {name: {variant: SOURCE_ROOT / variants[variant]
