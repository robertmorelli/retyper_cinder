...
# mask=58687486  (23/26 units erased)
...
import sys
from __static__ import cast, cbool, inline
...
class Packet(object):
    ...
    def append_to(self, lst: Any) -> Packet:
        self.link = None
        if lst is None:
            return self
        else:
...
class HandlerTaskRec(TaskRec):
    ...
    def workInAdd(self, p: Any) -> Optional[Packet]:
        self.work_in = p.append_to(self.work_in)
        return self.work_in

    def deviceInAdd(self, p: Any) -> Optional[Packet]:
...
class TaskState(object):
    ...
    def isTaskWaiting(self) -> cbool:
        return cbool(self.task_waiting)
    ...
    def isTaskHoldingOrWaiting(self) -> cbool:
        return cbool(self.task_holding or (not self.packet_pending and self.task_waiting))

    @inline
    def isWaitingWithPacket(self) -> cbool:
        return cbool(self.packet_pending and self.task_waiting and (not self.task_holding))
tracing: bool = False
layout = 0
...
class Task(TaskState):
    ...
    def __init__(self, i: Any, p: Any, w: Any, initialState: Any, r: Any) -> None:
        wa: Any = taskWorkArea
    ...
    def fn(self, pkt: Any, r: Any) -> Optional[Task]:
        raise NotImplementedError

    def addPacket(self, p: Any, old: Any) -> Task:
...
class DeviceTask(Task):
    ...
    def fn(self, pkt: Any, r: Any) -> Task:
...
class Richards(object):
    ...
    def run(self, iterations: Any) -> bool:
