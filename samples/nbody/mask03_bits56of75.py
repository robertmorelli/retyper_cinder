...
# mask=35167265693375148317631  (56/75 units erased)
...
class Vector:
    ...
    def __init__(self, x: double, y: Any, z: Any):
        self.x: Any = box(x)
        self.y: double = double(y)
        self.z: Any = z

    def __repr__(self) -> Any:
        return f'Vector({(self.x, box(self.y), self.z)})'

class Body:

    def __init__(self, pos: Vector, v: Any, mass: Any):
        self.pos: Any = pos
        self.v: Any = v
        self.mass: Any = mass
...
def report_energy(bodies: Any=SYSTEM, pairs: Any=PAIRS, e: double=0.0) -> Any:
    b1: Any
    b2: Any
    body: Any
    for b1, b2 in pairs:
        ...
        dz: Any = pos1.z - pos2.z
        e -= double(b1.mass * b2.mass / (dx * dx + box(dy * dy) + dz * dz) ** 0.5)
    for body in bodies:
        v: Any = body.v
        e += double(body.mass * (v.x * v.x + v.y * v.y + v.z * v.z)) / 2.0
    return box(e)

def offset_momentum(ref: Any, bodies: CheckedList[Body], px: Any=0.0, py: double=0.0, pz: Any=0.0) -> Any:
    body: Body
    for body in bodies:
        v: Any = body.v
...
def bench_nbody(loops: Any, reference: str, iterations: int):
