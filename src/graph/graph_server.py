...
fetch('/benchmarks').then(r=>r.json()).then(names=>{for(const name of names)selectBox.add(new Option(name,name));load()});selectBox.onchange=load;variantBox.onchange=load;functionBox.onchange=load;maskBox.oninput=schedule;proportionCheck.onchange=()=>proportionWrap.hidden=!proportionCheck.checked;proportion.oninput=()=>proportionValue.textContent=proportion.value+'%';generate.onclick=randomMask;source.onscroll=draw;onresize=draw;
</script>'''
