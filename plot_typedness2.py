"""Overlay statically compiled against plainly run, per benchmark.
...
def main():
    ...
    parser.add_argument("-o", "--out", default="typedness2.png")
    ...
    for ax, bench in zip(flat, benches):
        ...
        ax.set_title(f"{bench}  ({n} units)", fontsize=10, color=INK,
                     loc="left", pad=6)
        values = draw(ax, rows, "compiled", COMPILED, COMPILED_BAND,
                      "statically compiled")
    ...
    fig.suptitle("Benchmark time vs proportion of typedness: statically "
                 "compiled vs run directly, with injected coercions "
