...
# mask=1182272  (5/21 units erased)
...
class Task(TaskState):
    ...
    def __init__(self, i: int64, p: int64, w: Optional[Packet], initialState: TaskState, r: TaskRec) -> None:
        ...
        self.packet_pending = box(initialState.isPacketPending())
        self.task_waiting = box(initialState.isTaskWaiting())
        self.task_holding = box(initialState.isTaskHolding())
        self.handle = r
        wa.taskList = self
        wa.taskTab[box(i)] = self
    ...
    def qpkt(self, pkt: Any) -> Task:
        t: Any = Task.findtcb(self, int64(pkt.ident))
        taskWorkArea.qpktCount += 1
        pkt.link = None
        pkt.ident = box(self.ident)
...
class Richards(object):
    ...
    def run(self, iterations: Any) -> bool:
        for i in range(iterations):
            ...
            wkq: Any = Packet(None, 0, int64(K_WORK))
