...
def create_array(start, end, step) -> Array[int64]:
    """
    ...
    size = box((static_abs(end - start) - 1) // static_abs(step) + 1)
    a = Array[int64](size)
    while i < size:
        a[i] = int64(c)
        c = c + step
...
def permutations(pool, r=-1) -> Iterator[Array[int64]]:
    n = len(pool)
    if r == -1:
        r = n
    rb = r
    indices = create_array(0, n, 1)
    cycles = create_array(n, n - r, -1)
    per = Array[int64](rb)
    idx = 0
    while idx < r:
...
def solve(queen_count) -> Iterator[Array[int64]]:
    """N-Queens solver.
    ...
    static_cols = create_array(0, queen_count, 1)
