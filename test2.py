"""The run button at annotation granularity -- `python test2.py`.

Same job as test.py, but a unit is one annotation group rather than one
function group, which is what data/benchmark_granularity.json asks for on most
of the benchmarks. The spaces are far larger here (nbody 82 units, deltablue
193), so the level sweep alone is hundreds of masks per benchmark and
`--fuzz N` / `--no-levels` exist to cut a run down to a probe.
...
SEED = 8675309
REPETITIONS = 1
# `python test2.py --less-any` erases to a deleted annotation rather than to
# `Any`. It is a different program, so it keeps its own record of what breaks.
LESS_ANY = "--less-any" in argv
SUFFIX = ("_annotation" + ("_less_any" if LESS_ANY else ""))
