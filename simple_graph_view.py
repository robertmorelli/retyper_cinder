...
<script>const data=__DATA__,source=document.querySelector('#source'),edges=document.querySelector('#edges'),cells=document.querySelector('#cells'),info=document.querySelector('#info'),NS='http://www.w3.org/2000/svg',byId=new Map(data.nodes.map(n=>[n.id,n]));data.source.forEach((text,i)=>{const row=document.createElement('span');row.className='line';row.dataset.line=i+1;const no=document.createElement('span');no.className='no';no.textContent=i+1;const body=document.createElement('span');body.className='text';body.textContent=text||' ';row.append(no,body);source.append(row)});
function el(tag,attrs){const e=document.createElementNS(NS,tag);for(const [k,v] of Object.entries(attrs))e.setAttribute(k,v);return e}function position(n){const row=document.querySelector(`.line[data-line="${n.line}"]`),body=row.querySelector('.text'),r=body.getBoundingClientRect(),style=getComputedStyle(body),probe=document.createElement('canvas').getContext('2d');probe.font=style.font;const w=probe.measureText('M').width,x1=r.left+n.col*w,x2=r.left+Math.max(n.col+1,n.endline===n.line?n.endcol:n.col+1)*w;return{x:(x1+x2)/2,x1,x2,y:n.slot==='type'?r.bottom-3:r.top+3}}
...
def graph_data(source):
    graph = Binder().bind(ast.parse(source))
    cells = set(graph.labels)
    for edge in graph.edges:
        cells.update((edge.source, edge.target))
    ordered = sorted(cells, key=lambda cell: (
        getattr(cell[0], "lineno", 0), cell[1], graph.labels[cell]))
    ids = {cell: index for index, cell in enumerate(ordered)}
    anchor = lambda cell: graph.anchors.get(cell, cell[0])
    nodes = [{"id": ids[cell], "slot": cell[1],
              ...
              "label": graph.labels[cell],
              "values": sorted(graph.values[cell])} for cell in ordered]
