...
`python benchmarking/write_samples.py` and read the files. This is for looking at output
...
120s timeout. That is the mask harness's job. `--check` and `--run` ask for it anyway.
  ...
  python benchmarking/write_samples.py     benchmarking/samples/, 3 masks a benchmark
  python benchmarking/write_samples.py --count 15      more masks each
  python benchmarking/write_samples.py --benchmark deltablue --benchmark nbody
  python benchmarking/write_samples.py --check         compile each one too
"""
...
def render(bench, mask, n):
    ...
    body = unparse(detype(source, mask=mask, bench=True))
    bits = bin(mask).count("1")
...
def main():
    ...
    out_dir = args.out or path.join(ROOT, "benchmarking", "samples")
