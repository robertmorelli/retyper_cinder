...
# mask=130011  (14/18 units erased)
...
@inline
def first_set_of_size(size: int64) -> Any:
    return (1 << box(size)) - 1 << 1
...
@inline
def ctz(low_bit: int64, bit_indexes: Array[int64]) -> Any:
    return box(bit_indexes[low_bit])
...
def main() -> Any:
    reference_result: int64 = int64(run_reference_case())
    assert reference_result == 19
    random_node_count: int = RANDOM_NODE_COUNT
    start_time: float = time.time()
    random_result: int64 = int64(run_random_benchmark(random_node_count))
