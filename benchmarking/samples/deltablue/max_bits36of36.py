...
class UrnaryConstraint(Constraint):
    ...
    def choose_method(self, mark: Any) -> Any:
        if int64(self.my_output.mark) != int64(mark) and cbool(stronger(self.strength, self.my_output.walk_strength)):
            self.satisfied = True
        else:
            self.satisfied = False
...
class BinaryConstraint(Constraint):
    ...
    def choose_method(self, mark: Any) -> Any:
        if int64(self.v1.mark) == int64(mark):
            if int64(self.v2.mark) != int64(mark) and cbool(stronger(self.strength, self.v2.walk_strength)):
                self.direction = Direction.FORWARD
            else:
                self.direction = Direction.BACKWARD
        if int64(self.v2.mark) == int64(mark):
            if int64(self.v1.mark) != int64(mark) and cbool(stronger(self.strength, self.v1.walk_strength)):
                self.direction = Direction.BACKWARD
            else:
